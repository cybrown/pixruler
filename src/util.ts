...
export function measureFromPosition(imageData: ImageData, x: number, y: number): {
    top: number;
    bottom: number;
    left: number;
    right: number;
    ...
    return {
        bottom: getLastBottomSamePixel(imageData, x, y),
        height: getHeightAtPosition(imageData, x, y),
        left: getLastLeftSamePixel(imageData, x, y),
        right: getLastRightSamePixel(imageData, x, y),
        top: getLastTopSamePixel(imageData, x, y),
        width: getWidthAtPosition(imageData, x, y),
    };
}

function getLastTopSamePixel(imageData: ImageData, x: number, y: number) {
...
function getLastBottomSamePixel(imageData: ImageData, x: number, y: number) {
    const startPixel = getPixel(imageData, x, y);
    let yToCheck = y + 1;
    while (yToCheck != 0) {
        const currentPixel = getPixel(imageData, x, yToCheck);
...
function getWidthAtPosition(imageData: ImageData, x: number, y: number): number {
